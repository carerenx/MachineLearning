...
for each in flist:
    ...
    nb_classes = len(np.unique(y_test)) # 有几个类别，np.nique函数是去除数组中的重复数字
    batch_size = min(x_train.shape[0] / 10, 16) # 设定批的大小，//表示整除
    # 归一化
    ...
    x_train_mean = x_train.mean() #求均值
    x_train_std = x_train.std() #标准差

    x_train = (x_train - x_train_mean) / (x_train_std) #z-score标准化
    x_test = (x_test - x_train_mean) / (x_train_std)

    x_train = x_train.reshape(x_train.shape + (1, 1,))#加一行一列？
    x_test = x_test.reshape(x_test.shape + (1, 1,))

    x = keras.layers.Input(x_train.shape[1:]) #第一行作为第一层
